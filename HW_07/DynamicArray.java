...
public class DynamicArray {
    ...
    public void addArray(int[] arrAdd ) {
        ...
        while ((count +arrAdd.length) >= size) { // if size is not enough make size double
            growSize();
        }

        for (int i= 0; i < arrAdd.length;i++ ) {
            array[count+i] = arrAdd[i]; // insert data at given index
        }
...
public void addAtArray(int index, int[] arrAdd ) {
    ...
    while ((count + arrAdd.length) >= size) { // if size is not enough make size double
        ...
        for (int i = count - 1; i >= index; i--) {
                array[i + arrAdd.length ] = array[i]; // shift all element right from given index
   }

    for (int i = 0; i < arrAdd.length;i++ ) {
        array[index+i] = arrAdd[i]; // insert data at given index
    }
    count+=arrAdd.length;
    }
